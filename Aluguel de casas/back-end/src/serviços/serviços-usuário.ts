 ...
 import MoradorAdicional from "../entidades/moradoradicional";
 dotenv.config();
 const SALT = 10;
  ...
  static async verificarCpfExistente(request, response) {
   ...
   static async verificarCadastroCompleto(usuário: Usuário) {
    switch(usuário.perfil) {
      case Perfil.INQUILINO:
        const inquilino = await Inquilino.findOne({ where: { usuário: usuário.cpf },
          relations: ["usuário"] });
        if (!inquilino) return false;
        return true;
      case Perfil.MORADORADICIONAL:
        const morador_adicional = await MoradorAdicional.findOne({ where: { usuário: usuário.cpf },
          relations: ["usuário"] });
        if (!morador_adicional) return false;
        return true;
      default: return;
    }
  };
