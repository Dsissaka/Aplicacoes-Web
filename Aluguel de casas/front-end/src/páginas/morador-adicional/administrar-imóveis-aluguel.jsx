import { useContext, useEffect, useRef, useState } from "react";
 ...
 import { serviçoBuscarImóveisAluguelMoradorAdicional } from "../../serviços/serviços-morador-adicional";
 ...
 export default function AdministrarImóveisAluguel() {
  const referênciaToast = useRef(null);
  const { usuárioLogado } = useContext(ContextoUsuário);
  const { imóvelAluguelConsultado, setImóvelAluguelConsultado, setContratoLocaçãoSelecionada }
  ...
  function adicionarImóvelAluguel() {
    setImóvelAluguelConsultado(null);
    setContratoLocaçãoSelecionada(null);
  ...
  function BooleanBodyTemplate(interesse) {
    if (interesse.finalizada) return "Sim";
    else return "Não";
  };
  ...
  useEffect(() => {
    ...
    async function buscarImóveisAluguelMoradorAdicional() {
      try {
        const response = await serviçoBuscarImóveisAluguelMoradorAdicional(usuárioLogado.cpf);
    ...
    buscarImóveisAluguelMoradorAdicional
    ();
  ...
  return ( 
    <div className={estilizarFlex()}>
      ...
      <Card title="Administrar Interesses" className={estilizarCard(usuárioLogado.cor_tema)}>
        <DataTable dataKey="id" size="small" paginator rows={TAMANHOS.MAX_LINHAS_TABELA}
          ...
          <Column bodyClassName={estilizarColunaConsultar()} body={ConsultarTemplate}
            headerClassName={estilizarColumnHeader(usuárioLogado.cor_tema)}/>
             <Column field="contrato.inquilino.usuário.nome" header="Inquilino" filter 
          ...
          <Column headerClassName={estilizarColumnHeader(usuárioLogado.cor_tema)}
            field="contrato.status" header="Categoria de moradia" filter filterMatchMode="equals"
          ...
          <Column field="contrato.título" header="Proposta" filter showFilterOperator={false}
            headerClassName={estilizarColumnHeader(usuárioLogado.cor_tema)} sortable/>
          <Column field="contrato.finalizada" header="Proposta finalizada ?" dataType="boolean" filter 
        ...
        <Button className={estilizarBotão()} label="Adicionar" onClick={adicionarImóvelAluguel}/>
      </Card>
    </div>
  );
 }